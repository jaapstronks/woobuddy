...
- [x] `docs/plans/TODO.md`: philosophy bullet #4 rewritten (no more "the one unavoidable server touchpoint for content"); new bullet #1 makes the no-LLM stance explicit; Phase B gets todos #12–#17 (pivot work renumbered to the lowest available slots), Phase C gets #19 (redaction log, promoted), #20 (bulk sweeps), #21 (per-document wordlist); "Briefings Not Adopted" section documents the Tier 2 and Tier 3 LLM deactivation.
- [x] `docs/plans/briefs/WOO_BUDDY_TODO.md`: moved to `docs/plans/done/WOO_BUDDY_TODO.md` with a "HISTORICAL — dormant LLM setup" banner at the top. Nobody should need to install Ollama to run the app.
- [x] `docs/plans/done/15-tier2-suggestion-ux.md`: pivot note added to the frontmatter explaining that `subject_role` and reason strings now come from the rule engine, not an LLM verdict.
- [x] `docs/plans/done/19-redaction-log.md`: frontmatter promoted to P1, Phase C; pivot note added explaining why (analyse.md identifies audit trail as the headline sales feature).
...
- `.env.example`
- `docs/plans/TODO.md`
- `docs/plans/done/15-tier2-suggestion-ux.md`
- `docs/plans/done/19-redaction-log.md`
- `docs/plans/done/WOO_BUDDY_TODO.md` (moved from `docs/plans/briefs/`)
- `docs/plans/done/35-deactivate-llm.md` (new — this file)
- `docs/plans/done/12-name-lists-meertens-cbs.md` (new)
- `docs/plans/done/13-functietitel-publiek-functionaris.md` (new)
- `docs/plans/done/14-structuurherkenning.md` (new)
- `docs/plans/done/16-tier1-gaps.md` (new)
- `docs/plans/done/17-publieke-functionarissen-referentielijst.md` (new)
- `docs/plans/done/20-bulk-sweep-flows.md` (new)
- `docs/plans/done/21-per-document-custom-wordlist.md` (new)
