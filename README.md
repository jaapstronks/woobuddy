...
PDF's verlaten nooit de browser van de gebruiker. De server slaat geen documentinhoud op: tekstextractie gebeurt client-side via pdf.js, de rule-based analyse is vluchtig, en alleen beslissingen (bbox-coördinaten, entiteitstype, tier, artikel) worden in de database bewaard. Zie [`docs/reference/client-first-architecture.md`](docs/reference/client-first-architecture.md) voor de volledige specificatie.
...
- **Client-first is heilig.** Als een feature lijkt te vereisen dat PDF's op de server worden opgeslagen, is het ontwerp fout. Zie [`docs/reference/client-first-architecture.md`](docs/reference/client-first-architecture.md).
...
Wat er open staat, staat in de [issues](https://github.com/jaapstronks/woobuddy/issues). De interne worklist en de uitgewerkte briefs houden we buiten deze repo.
